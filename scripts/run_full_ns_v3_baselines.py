...
def main():
    ...
    artifact = json.loads((ROOT/'outputs/spider/hit_full_ns_v2.json').read_text())
    ...
    for region in REGIONS:
        ...
        loaders = create_hit_ns_v2_dataloaders(config)[region]
        for seed in SEEDS:
            for name, function in (
                ...
                path = ROOT/'outputs/runs'/f'full_ns_v3_{name}_{region}_seed{seed}'/'metrics.json'
        ...
        for name, source in (('noprop_vanilla', 'none'), ('pi_noprop', 'discovered')):
            ...
            for seed in SEEDS:
                path = (ROOT/'outputs/runs'/
                        f'full_ns_v3_{source}_{region}_lambda{weight}_seed{seed}'/
                        'metrics.json')
    ...
    aggregate = {'schema_version': 1, 'protocol': {'seeds': list(SEEDS),
                 'trajectory_disjoint': True}, 'results': {}}
    ...
    output = ROOT/'outputs/aggregate/full_ns_v3_baselines.json'

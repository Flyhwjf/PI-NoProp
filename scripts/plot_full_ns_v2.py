"""Generate manuscript figures directly from the full-NS v2 artifacts."""
...
def local_update():
    ...
    save(fig, 'fig_v2_local_training.png')
...
def dns_quality(manifest):
    ...
    fig.tight_layout(); save(fig, 'fig_v2_dns_quality.png')


def data_samples():
    root = ROOT/'data/cache_hit_ns_v2'
    ...
    save(fig, 'fig_v2_data_samples.png')
...
def spider_figure(artifact):
    ...
    fig.tight_layout(); save(fig, 'fig_v2_spider.png')
...
def result_figures(aggregate):
    ...
    save(fig, 'fig_v2_main_results.png')
    ...
    axes[0].legend(frameon=False); fig.tight_layout(); save(fig, 'fig_v2_efficiency.png')
...
def noise_figure(artifact):
    ...
    fig.tight_layout(); save(fig, 'fig_v3_noise.png')


def main():
    style(); framework(); local_update()
    manifest = json.loads((ROOT/'data/generated_hit_ns_v2/manifest.json')
                          .read_text(encoding='utf-8'))
    artifact = json.loads((ROOT/'outputs/spider/hit_full_ns_v2.json')
                          .read_text(encoding='utf-8'))
    dns_quality(manifest); data_samples(); spider_figure(artifact)
    aggregate_path = ROOT/'outputs/aggregate/full_ns_v3_results.json'
    ...
    noise_path = ROOT/'outputs/aggregate/full_ns_v3_noise.json'

...
def main():
    baselines = json.loads(Path(
        'outputs/aggregate/full_ns_v3_baselines.json').read_text())
    ...
    ablation = json.loads(Path(
        'outputs/aggregate/full_ns_v3_equation_ablation.json').read_text())
    ...
    noise = json.loads(Path('outputs/aggregate/full_ns_v3_noise.json').read_text())
    ...
    spider = json.loads(Path(
        'outputs/aggregate/full_ns_v3_spider_noise.json').read_text())
    ...
    for token in ('tab:expanded-baselines', 'tab:equation-ablation',
                  'tab:noise-prediction', 'tab:spider-noise', 'fig_v3_noise.png'):
